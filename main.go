// Pafu detects slaps/hits on the laptop and plays audio responses.
...
var (
	...
	// Runtime metadata exposed to GUI clients via stdio.
	// Set once when listenForSlaps starts; read-only afterwards.
	currentMode   string
...
func main() {
	cmd := &cobra.Command{
		Use:   "pafu",
		Short: "Pafu yells back when you slap the laptop",
		Long: `Pafu reads the Apple Silicon accelerometer directly via IOKit HID
...
within a minute, the more intense the sounds become.`,
	...
	cmd.Flags().BoolVar(&serveMode, "serve", false, "Enable HTTP server mode (REST + SSE on 127.0.0.1) for browser/web integrations")
	cmd.Flags().IntVar(&serverPort, "port", 17878, "Port for HTTP server when --serve is enabled")
...
func run(ctx context.Context, tuning runtimeTuning) error {
	if os.Geteuid() != 0 {
		return fmt.Errorf("pafu requires root privileges for accelerometer access, run with: sudo pafu")
	}
	...
	if err := buildPackRegistry(initialPackID, pack, tuning.cooldown); err != nil {
		return err
	}
	...
	// If serve mode is enabled, spin up an in-process HTTP server.
	// The detection loop runs in this goroutine; the HTTP server runs
	// in its own. SSE subscribers receive events via globalBus, which
	// listenForSlaps writes to.
	if serveMode {
		globalBus = newEventBus()
		go func() {
			if err := runHTTPServer(ctx, serverPort); err != nil {
				fmt.Fprintf(os.Stderr, "pafu: http server error: %v\n", err)
			}
		}()
	}
...
func listenForSlaps(ctx context.Context, pack *soundPack, accelRing *shm.RingBuffer, tuning runtimeTuning) error {
	...
	if stdioMode {
		stdioReady := make(map[string]interface{}, len(readyPayload)+1)
		stdioReady["status"] = "ready"
		for k, v := range readyPayload {
			stdioReady[k] = v
		}
		if data, err := json.Marshal(stdioReady); err == nil {
			fmt.Println(string(data))
		}
	} else {
		fmt.Printf("pafu: listening for slaps in %s mode with %s tuning... (ctrl+c to quit)\n", pack.name, presetLabel)
	}
	publishBus("ready", readyPayload)
	...
	for {
		...
		case <-ctx.Done():
			if stdioMode {
				fmt.Println(`{"status":"bye"}`)
			} else {
				fmt.Println("\nbye!")
			}
			publishBus("bye", map[string]interface{}{})
			return nil
		case err := <-sensorErr:
			return fmt.Errorf("sensor worker failed: %w", err)
		case <-ticker.C:
		...
		_, activePack, tracker := activePackSnapshot()
		if activePack == nil || tracker == nil {
			continue
		}
		...
		publishBus("slap", event)
		go playAudio(activePack, file, ev.Amplitude, &speakerInit)
	}
}
...
func playAudio(pack *soundPack, path string, amplitude float64, speakerInit *bool) {
	...
	if pack.custom {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "pafu: open %s: %v\n", path, err)
			return
		}
		defer file.Close()
		streamer, format, err = mp3.Decode(file)
		if err != nil {
			fmt.Fprintf(os.Stderr, "pafu: decode %s: %v\n", path, err)
			return
		}
	} else {
		data, err := pack.fs.ReadFile(path)
		if err != nil {
			fmt.Fprintf(os.Stderr, "pafu: read %s: %v\n", path, err)
			return
		}
...
// VolumeScaling uses a pointer so we can distinguish
// "field omitted" from "explicit false".
...
// writeJSON marshals v and writes a single line of JSON to w.
// On marshal error, falls back to a generic error message so the
// stream is never broken.
func writeJSON(w io.Writer, v interface{}) {
	data, err := json.Marshal(v)
	if err != nil {
		fmt.Fprintln(w, `{"error":"internal marshal error"}`)
		return
	}
...
func processCommands(r io.Reader, w io.Writer) {
	...
	for scanner.Scan() {
		...
		case "set":
			...
			applySettings(ampPtr, cdPtr, spPtr, cmd.VolumeScaling)
			if stdioMode {
